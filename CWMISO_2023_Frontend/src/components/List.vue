  ...
  <v-col cols="12">
    <v-card class="pa-3 mb-2 mr-2 rounded-xl" elevation="0" color='grey-lighten-4'
      ...
      <v-row no-gutters justify="center">
        ...
        <v-col class="text-center pt-2 pt-lg-0 d-flex align-center justify-center" no-gutters>
          <v-span>
            <!-- แก้ไข เรียกใช้ปุ่มเลือก หน้า AdminUser -->
            <v-btn color="cyan-darken-2" class="mr-1" size="40" icon="" >
              <v-icon icon="mdi-label-multiple-outline"></v-icon>
              <v-tooltip activator="parent" location="bottom">เลือก</v-tooltip>
            </v-btn>

            <v-btn color="grey-darken-2" class="mr-1" size="40" icon="" >
              <v-icon icon="mdi mdi-cog"></v-icon>
              <v-tooltip activator="parent" location="bottom">ตั้งค่า</v-tooltip>
            </v-btn>

            <v-btn color="blue-accent-3" class="mr-1" size="40" icon="" @click="ShowDialog('detail'), GetID(layout.id[layout_data_index])">

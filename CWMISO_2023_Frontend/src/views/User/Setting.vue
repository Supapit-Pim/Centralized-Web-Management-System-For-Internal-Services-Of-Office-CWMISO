...
import { useUserStore, useTokenStore, useCompanyStore, PiniaRoot } from '@/stores/'
                    ...
                    <v-row no-gutters class="d-flex justify-center align-center">
                      ...
                      <v-col cols="12">
                        <v-row class="ma-0">
                          <v-col class="space-y-1">
                            <h2>ชื่อ-นามสกุล</h2>
                            <v-text-field v-model="nameUser" :rules="validationName" :readonly="loadingupdate"
                              name="name" type="text">
                            </v-text-field>
                          </v-col>
                          ...
                          <v-col class="space-y-1">
                            <h2>ชื่อผู้ใช้งาน</h2>
                            <v-text-field v-model="usernameUser" :rules="validationUsername" :readonly="loadingupdate"
                          ...
                          <v-col class="space-y-1">
                            <h2>รหัสผ่าน</h2>
                            <v-text-field v-model="passwordUser" :rules="validationPassword" :readonly="loadingupdate"
...
export default {
  ...
  watch: {},
  methods: {
    ...mapActions(useUserStore, ['Get_User_ID_U', 'Update_User_ID_U']),
    ...mapActions(useCompanyStore, ['Get_Company_ID_U']),
    ...
    async SettingUserGetID() {
      ...
      await useUserStore().Get_User_ID_U(PiniaRoot.token.token, this.data_token.id)
      const response_user = await useUserStore().$state.data_id
      
      typeof response_user == 'string'
        ? await sweetalert2.warning(response_user)
        : this.data_store = await response_user[0]

      await useCompanyStore().Get_Company_ID_U(PiniaRoot.token.token, this.data_token.company_id)
      ...
      this.nameUser = this.data_store.name
      this.usernameUser = this.data_store.username
      this.passwordUser = null
      this.photoUser = this.data_store.photo
    },
    async onSubmitupdate() {
      ...
      const formData = {
        ...
        photo: this.base64String == null ? this.photoUser : this.base64String,
      ...
      await sweetalert2.Delete("ยืนยันเพื่อ ออกจากระบบ", '', '', async res => {
        if (res[0].isConfirmed == true) {
          const response = await this.Update_User_ID_U(PiniaRoot.token.token, update_data, this.data_token.id)
          if (typeof response == 'string') {
            await sweetalert2.warning(response)
          } else {
            await sweetalert2.success('สำเร็จ')
            UserStore.$state.data_id = update_data

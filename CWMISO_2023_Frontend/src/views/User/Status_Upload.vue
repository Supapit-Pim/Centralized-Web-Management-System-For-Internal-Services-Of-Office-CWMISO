...
import { useStatusUploadStore, PiniaRoot } from '@/stores/'
...
<template>
  <div class="p-5 space-y-5 w-full h-full">
    <h1 class="text-3xl">สถานะอัปโหลด</h1>
    <!-- Button Search -->
    ...
    <v-row no-gutters>
      <v-col
        v-for="(data, index) in status_upload"
        :key="index"
        :id="'data_' + data.id"
        cols="12"
        md="6"
      >
        <v-card class="pa-2 mb-4 mr-4 rounded-xl" elevation="0" >
          <v-row>
            ...
            <v-col cols="5">
              <v-container>
                <v-span class="text-xs text-sm-caption text-md-body-2">ประเภทไฟล์ทั้งหมด : {{ data.type_all_id }}</v-span><br />
                <v-span class="text-xs text-sm-caption text-md-body-2">วันที่สร้าง : {{ data.create_time }}
                </v-span>
              </v-container>
            </v-col>
            ...
            <v-col cols="5" class="text-center">
              <v-container class="pa-2">
                <v-btn
                  class="rounded-lg"
                  color="primary"
                  @click="StatusUploadGetID(data.id), (dialog = true)"
                >
                  <v-icon start icon="mdi-text-box-edit-outline"></v-icon> รายละเอียด
                </v-btn>
                <v-btn class="rounded-lg ma-4" @click="data.id;" color="red">
              ...
              <v-window v-model="tab">
                ...
                <v-window-item value="one">
                  <v-row>
                    ...
                    <v-col cols="12" sm="6" md="4">
                      <div class="text-h7 mb-1">ประเภทไฟล์ทั้งหมด</div>
                      <v-text-field>
                    ...
                    <v-col cols="12" sm="6" md="4">
                      <div class="text-h7 mb-1">รายการอัปโหลด</div>
                      <v-text-field>
                        <p>{{ data_id.list_upload_id }}</p>
                      </v-text-field>
                    </v-col>
                    ...
                    <v-col cols="12" sm="6">
                      <div class="text-h7 mb-1">ผู้อัปโหลดตำแหน่ง User</div>
                      <v-text-field>
                        <p>{{ data_id.user_id }}</p>
                      </v-text-field>
                    </v-col>
    ...
    <!-- <div class="flex flex-wrap align-center justify-cente p-2">
            <div class="rounded-md m-2 bg-slate-300 w-auto h-auto p-2" v-for="(data, index) in status_upload" :key="index">
                <p>id : {{ data.id }}</p>
                <p>type_all_id : {{ data.type_all_id }}</p>
                <p>list_upload_id : {{ data.list_upload_id }}</p>
...
export default {
  ...
  async created() {
    this.StatusUploadStore()
  },
  methods: {
    ...mapActions(useStatusUploadStore, ['GetDataStatusUpload', 'Getdata', 'GetID', 'Delete']),
    async StatusUploadStore() {
      const StatusUploadStore = useStatusUploadStore()
      await this.Getdata(PiniaRoot.token.token)
      this.status_upload = StatusUploadStore.data
      this.list = this.status_upload
    },
    async StatusUploadGetID(id) {
      const StatusUploadStore = useStatusUploadStore()
      this.data_id = StatusUploadStore.$state.data.find((p) => p.id == id)
    }
  }
}
</script>

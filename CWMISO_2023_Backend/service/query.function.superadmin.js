const controller = require("./controller.function");
class Query extends controller {
  rawQuery(sql, values = null) {
    return new Promise((resolve, reject) => {
      this.connection.query(sql, [values], (error, results) => {
        error
          ? resolve(error.message)
          : resolve(results);
      });
    });
  }
  async getQuery(pathpermission, CRUD, req, jwt, table, type_query = "*", where = "",
    group_by = "", order_by = "", limit = "") {
    const sql = `SELECT ${type_query} FROM ${table} ${group_by} ${where} ${order_by} ${limit}`;
    console.log("sql",sql);
    if (jwt == 1) {
      const checkJWT = await this.CheckJWT_Superadmin(pathpermission, table, req, CRUD);
      if (typeof checkJWT !== "string") {
        const SQL = await this.rawQuery(sql)
        return SQL != ""
          ? SQL
          : "Not have query data";
      } else {
        return checkJWT
      }
    }
    else { 
      const SQL = await this.rawQuery(sql)
      return SQL != ""
        ? SQL
        : "Not have query data";
    }
  }
  async insertQuery(pathpermission, CRUD, req, jwt, table, data) {
    const sql = ` INSERT INTO ${table} SET ? , delete_status = 0`;
    if (jwt == 1) {
      const checkJWT = await this.CheckJWT_Superadmin(pathpermission, table, req, CRUD);
      if (typeof checkJWT !== "string") {
        if (table != "admin") {
          const SQL = await this.rawQuery(sql, data)
          return SQL != ""
            ? SQL
            : "Not have query data";
        } else {
          const where_company = await this.getQuery("", "", "", 0, table, "*", `WHERE company_id = ${req.body.company_id}`)
          if (typeof where_company == "string") {
            const SQL = await this.rawQuery(sql, data)
            return SQL != "" && typeof SQL != "string"
              ? SQL
              : "Not have query data";
          } else {
            return `Company id ${req.body.company_id} have user admin`
          }
        }
      } else {
        return checkJWT
      }
    } else {
      return this.rawQuery(sql, data);
    }
  }
  async updateQuery(pathpermission, CRUD, req, jwt, table, data, where,) {
    const sql = `UPDATE ${table} SET ? ${where}`;
    if (jwt == 1) {
      const checkJWT = await this.CheckJWT_Superadmin(pathpermission, table, req, CRUD);
      if (typeof checkJWT !== "string") {
        if (table != "admin") {
          const SQL = await this.rawQuery(sql, data)
          return SQL != ""
            ? SQL.affectedRows != 0
              ? SQL
              : `SQL Rows matched = ${SQL.affectedRows}`
            : "Not have query data";
        } else {
          const where_company = await this.getQuery("", "", "", 0, table, "*", `WHERE company_id = ${req.body.company_id}`);
          if (where_company == "") {
            const SQL = await this.rawQuery(sql, data)
            return SQL != ""
            ? SQL.affectedRows != 0
              ? SQL
              : `SQL Rows matched = ${SQL.affectedRows}`
            : "Not have query data";
          } else {
            return `Company id ${req.body.company_id} have user admin`
          }
        }
      } else {
        return checkJWT
      }
    } else {
      return this.rawQuery(sql);
    }
  }
  async updateStatusDelete(pathpermission, CRUD, req, jwt, table, where) {
    const sql = `UPDATE ${table} SET delete_status = 1 ${where}`;
    if (jwt == 1) {
      const checkJWT = await this.CheckJWT_Superadmin(pathpermission, table, req, CRUD);
      if (typeof checkJWT !== "string") {
        const SQL = await this.rawQuery(sql)
        return SQL != ""
        ? SQL.affectedRows != 0
          ? SQL
          : `SQL Rows matched = ${SQL.affectedRows}`
        : "Not have query data"
      } else {
        return checkJWT
      }
    } else {
      return this.rawQuery(sql);
    }
  }
  async updateStatusUnDelete(pathpermission, CRUD, req, jwt, table, where) {
    const sql = `UPDATE ${table} SET delete_status = 0 ${where}`;
    if (jwt == 1) {
      const checkJWT = await this.CheckJWT_Superadmin(pathpermission, table, req, CRUD);
      if (typeof checkJWT !== "string") {
        const SQL = await this.rawQuery(sql)
        return SQL != ""
        ? SQL.affectedRows != 0
          ? SQL
          : `SQL Rows matched = ${SQL.affectedRows}`
        : "Not have query data"
      } else {
        return checkJWT
      }
    } else {
      return this.rawQuery(sql);
    }
  }
  async deleteQuery(pathpermission, CRUD, req, jwt, table, where) {
    const sql = `DELETE FROM ${table} ${where}`;
    if (jwt == 1) {
      const checkJWT = await this.CheckJWT_Superadmin(pathpermission, table, req, CRUD);
      if (typeof checkJWT !== "string") {
        const SQL = await this.rawQuery(sql)
        return SQL != ""
            ? SQL.affectedRows != 0
              ? SQL
              : `SQL Rows matched = ${SQL.affectedRows}`
            : "Not have query data"
      } else {
        return checkJWT
      }
    } else {
      return this.rawQuery(sql);
    }
  }
  async joinQuery(req, jwt, tables, join_conditionsm, columns, type_join, group_by = "", having = "", order_by = "") {
    const joinTables = tables.join(` ${type_join} `);
    const selectColumns = columns.join(" , ");
    const joinSql = `SELECT ${selectColumns} FROM ${joinTables} ON ${join_conditionsm}  ${group_by}  ${having} ${order_by};`;
    if (jwt == 1) {
      const checkJWT = await this.CheckJWT_Superadmin(tables, req);
      if (typeof checkJWT !== "string") {
        const SQL = await this.rawQuery(joinSql)
        return SQL != ""
          ? SQL.affectedRows != 0
            ? SQL
            : `SQL Rows matched = ${SQL.affectedRows}`
          : "Not have query join data";
      } else {
        return checkJWT
      }
    } else {
      return this.rawQuery(joinSql);
    }
  }
  async UnionQuery(tableConditions, columns, NameAlias, aliasNames) {
    const unionQueries = tableConditions.map((condition, index) => {
      const { table, where } = condition;
      var alias = ""
      var tset = ""
      NameAlias.forEach(function (element, index2) {
        if (index2 === 1) {
          tset = " , "
        }
        alias += `${tset} ${aliasNames[index2][index]} AS ${NameAlias[index2]}`
      });
      return `SELECT ${columns.join(', ')},${alias} FROM ${table} ${where}`;
    });
    const queryString = unionQueries.join(' UNION ');
    const queryRoles = await this.rawQuery(queryString);
    return queryRoles;
  }
}
module.exports = Query;

    ...
    if (jwt == 1) {
      ...
      if (typeof checkJWT !== "string") {
        ...
        if (typeof CheckAdmin != "string") {
          ...
          const where_company = `AND company_id = ${admin.company_id}`
          const sqlNew = (table != "company" && table != "list_upload" && table != "type_company" && table != "type_all" && table != "type_social")
            ? `SELECT ${type_query} FROM ${table} ${group_by} ${where} ${where_company} ${order_by} ${limit}`
            : sql;
    ...
    if (jwt == 1) {
      ...
      if (typeof checkJWT !== "string") {
        ...
        if (typeof CheckAdmin != "string") {
          ...
          if (table != "list_upload" && table != "status_company" && table != "create_form" && table != "data_form" && table != "data_personnel" && table != "status_upload") {
            const where_company = `AND company_id = ${admin.company_id}`
            ...
            const where_company = `${joinWhere} AND company_id = ${admin.company_id}`
  ...
  async joinQuery(pathpermission, CRUD = "", req, jwt, table, tables, columns, join_conditions, type_join, group_by = "", where = "", having = "", order_by = "") {
    ...
    if (jwt == 1) {
      ...
      if (typeof checkJWT !== "string") {
        const admin = checkJWT[0]
        const where_company = `AND company_id = ${admin.company_id}`
        const joinSqlNew = `SELECT ${selectColumns} FROM ${joinTables} ON ${join_conditions} ${group_by} ${where} ${where_company} ${having} ${order_by};`;

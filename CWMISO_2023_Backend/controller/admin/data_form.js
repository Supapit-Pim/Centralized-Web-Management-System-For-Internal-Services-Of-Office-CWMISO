...
const tableName = query.Table_Data_Form;
const Where_Delete = query.Where_Delete;
const Where_UnDelete = query.Where_UnDelete;

const joinTable = [tableName, "create_form", "list_service", "company"];
const joinColumns = [`${tableName}.*`, `company.id AS company_id`];
const joinConditions = `${tableName}.create_form_id = create_form.id AND create_form.list_service_id = list_service.id AND list_service.company_id = company.id`;

const DataFormController = {
